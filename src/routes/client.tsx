      ...
      <div className="min-h-screen bg-zinc-950 text-white antialiased flex flex-col justify-between">
        ...
        <header className="border-b border-zinc-900/60 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-40 pt-[calc(28px+env(safe-area-inset-top,0px))] sm:pt-3 pb-3">
          <div className="mx-auto max-w-lg px-4 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-extrabold tracking-tight">DoctorCorpo GO</span>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 mx-auto w-full max-w-lg px-4 py-12 flex flex-col justify-center items-center text-center">
          ...
          <p className="text-zinc-400 text-xs mt-3 max-w-xs leading-relaxed">
            Esta clínica não foi encontrada em nossa base de dados ou foi desativada pelo administrador.
          </p>
          <button
        ...
        <footer className="py-4 text-center text-[10px] text-zinc-600 border-t border-zinc-900 bg-zinc-950">
          Desenvolvido para DoctorCorpo GO — Todos os direitos reservados.
        </footer>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-white antialiased flex flex-col justify-between">
          ...
          <div className="mx-auto max-w-lg flex items-center justify-between px-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-extrabold tracking-tight">
                {shopProfile ? shopProfile.name : "DoctorCorpo GO"}
              </span>
            </div>
            <div className="flex items-center gap-2.5">
              <span className="text-xs text-zinc-400 font-medium flex flex-col items-end">
                <span>Olá, <span className="text-amber-400 font-black">{session.name}</span></span>
                <span className="text-[9px] text-amber-300/80 font-mono tracking-tight mt-0.5">{session.phone}</span>
              </span>
              <button
        ...
        <main className="flex-1 mx-auto w-full max-w-lg px-4 py-12 flex flex-col justify-center items-center text-center">
          ...
          <p className="text-zinc-400 text-xs mt-3 max-w-xs leading-relaxed">
            Esta clínica está temporariamente indisponível para novos agendamentos online. Por favor, entre em contato diretamente com o estabelecimento para mais informações.
          </p>
          <a
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-extrabold tracking-tight">
              {shopProfile ? shopProfile.name : (
                <>DoctorCorpo <span className="text-amber-500">GO</span></>
              )}
            </span>
          </div>
          <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className={`flex-1 rounded-xl py-2.5 text-xs font-bold transition-all border ${
              activeTab === "book"
                ...
                : "bg-zinc-900/60 text-amber-400/80 border-amber-500/15 shadow-[0_0_10px_rgba(20,184,166,0.06)] hover:text-white hover:bg-zinc-800/60 hover:border-amber-500/30"
            }`}
          >
            Agendar Horário
          </button>
...
// Circular logo component for rendering either custom clinic logo or default logo
...
function BookingFlow({ clientPhone, clientName, shopProfile, onSessionUpdate, onLogout }: BookingFlowProps) {
  ...
  return (
    <div className="bg-zinc-950 py-2">
      ...
      {step === "barber" && (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          <h2 className="text-sm font-bold text-zinc-300 mb-3 uppercase tracking-wider">Escolha o Especialista</h2>
          
          {selectedService && (
            <div className="mb-4 bg-zinc-900/30 border border-zinc-800/40 rounded-xl p-3 flex items-center gap-3">
              <ShopCircleLogo profile={shopProfile} />
              <div>
                <p className="text-[9px] uppercase tracking-wider text-amber-400 font-extrabold">Procedimento Selecionado</p>
            ...
            {barbers.map((barber) => (
              ...
              >
                ...
                <span className={`text-xs font-black tracking-wider uppercase ${
                  selectedBarber?.id === barber.id ? "text-zinc-950" : "text-amber-400"
                }`}>
                  {barber.name}
                </span>
                <span className="text-[10px] opacity-65 mt-1 font-bold">Especialista</span>
              </button>
            ))}
          </div>
          ...
          <div className="mt-6 flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl bg-amber-600 text-zinc-950 hover:bg-amber-500 transition-all py-3.5 text-xs font-black active:scale-95 shadow-md shadow-amber-500/10 cursor-pointer"
            >
              Voltar
            </button>
          </div>
                ...
                if (activeSlots.length === 0) {
                  return (
                    <p className="text-center text-xs text-zinc-500 col-span-5 py-4 font-semibold">
                      Sem horários disponíveis para o turno deste profissional neste dia.
                    </p>
                  );
                }
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-5 space-y-4 shadow-xl shadow-black/20">
            <div className="flex items-center gap-3 pb-3 border-b border-zinc-800">
              <ShopCircleLogo profile={shopProfile} />
              <div>
                <p className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">Procedimento</p>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">Especialista</p>
              ...
              <div>
                ...
                <p className="font-bold text-amber-400 mt-0.5">{formatDuration(selectedService.duration)}</p>
              </div>
              <div>
            ...
            <div className="pt-3 border-t border-zinc-800 text-xs space-y-2.5">
              <div className="flex justify-between">
                <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">Nome do Cliente</span>
                <span className="font-bold text-amber-400 uppercase">{customClientName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">WhatsApp (com DDD)</span>
                <span className="font-bold text-amber-400">{customClientPhone}</span>
              </div>
            </div>
          </div>
      ...
      {step === "success" && selectedService && selectedBarber && (
        <div className="text-center py-6 animate-in zoom-in-95 duration-300">
          ...
          <p className="text-zinc-400 text-xs mt-1.5 px-4">
            Seu horário foi agendado e enviado para a clínica. Te esperamos lá!
          </p>
