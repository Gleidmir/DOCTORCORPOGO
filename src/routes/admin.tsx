  ...
  useEffect(() => {
    const checkAuth = async () => {
      if (isSupabaseConfigured) {
        ...
        setCurrentUser({
          role: "admin",
          name: session.user?.user_metadata?.name || "Administrador da Clínica",
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-white antialiased flex flex-col justify-between relative">
        ...
        <footer className="py-4 text-center text-[10px] text-zinc-600 border-t border-zinc-900 bg-zinc-950">
          Painel de Gerenciamento DoctorCorpo GO — Todos os direitos reservados.
        </footer>
      </div>
    );
  }
        ...
        <main className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-zinc-900 border border-amber-500/20 rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="space-y-1 text-left">
                  ...
                  <p className="text-[11px] font-mono text-zinc-500 mt-1 select-all break-all">
                    {typeof window !== "undefined" ? `${window.location.origin}/client?t=${session?.email ? session.email.split("@")[0] : "default"}${shopLogoUrl ? `&v=${shopLogoUrl.replace(/[^a-zA-Z0-9]/g, "").slice(-8)}` : ""}` : ""}
                  </p>
                </div>
                <div className="flex gap-2 w-full md:w-auto shrink-0">
                  <button
                    onClick={() => {
                      const cleanTenant = session?.email ? session.email.split("@")[0] : "default";
                      const cleanLogo = shopLogoUrl ? shopLogoUrl.replace(/[^a-zA-Z0-9]/g, "").slice(-8) : "";
                      const logoVersion = cleanLogo ? `&v=${cleanLogo}` : "";
                      const url = `${window.location.origin}/client?t=${cleanTenant}${logoVersion}`;
                  ...
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(
                      shopName
                        ? `Olá! Agende sua avaliação na clínica: *${shopName.toUpperCase()}* online pelo link: ${typeof window !== "undefined" ? window.location.origin : ""}/client?t=${session?.email ? session.email.split("@")[0] : "default"}${shopLogoUrl ? `&v=${shopLogoUrl.replace(/[^a-zA-Z0-9]/g, "").slice(-8)}3` : "&v=3"}`
                        : `Olá! Agende sua avaliação na nossa clínica online pelo link: ${typeof window !== "undefined" ? window.location.origin : ""}/client?t=${session?.email ? session.email.split("@")[0] : "default"}${shopLogoUrl ? `&v=${shopLogoUrl.replace(/[^a-zA-Z0-9]/g, "").slice(-8)}3` : "&v=3"}`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
          ...
          {activeTab === "barbeiros" && (
            <div className="space-y-4">
              <div className="flex justify-between items-center glass-card rounded-2xl p-4">
                <div>
                  <h2 className="text-sm font-bold text-zinc-300 uppercase tracking-wider">Gerenciador de Especialistas</h2>
                  <p className="text-[11px] text-zinc-500 mt-0.5">Adicione, edite ou remova profissionais da sua clínica.</p>
                </div>
                {!showBarberForm && (
                  <button
                    onClick={() => {
          ...
          {activeTab === "perfil" && (
            <div className="space-y-6 animate-fade-in text-left">
              <div className="space-y-1">
                <h2 className="text-xl font-extrabold text-white">Minha Clínica</h2>
                <p className="text-xs text-zinc-400">
                  Personalize as informações de visualização da sua clínica para seus clientes.
                </p>
              </div>
                    ...
                    <div className="flex flex-col items-center space-y-3">
                      ...
                      <div className="text-center w-full">
                        <h3 className="text-xs font-black text-white truncate max-w-full">
                          {shopName.trim() || "Minha Clínica"}
                        </h3>
                        <span className="text-[9px] text-zinc-500 font-semibold uppercase tracking-wider mt-0.5 block">DoctorCorpo GO</span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl max-w-4xl w-full p-6 sm:p-8 relative my-8 max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="text-center max-w-2xl mx-auto mb-6">
              ...
              <p className="text-zinc-400 text-xs mt-1">
                Selecione um plano e realize o pagamento para prolongar o acesso da sua clínica.
              </p>
            </div>
